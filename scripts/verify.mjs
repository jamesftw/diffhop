...
async function expectNoRedirect(name, url) {
  try {
    ...
    record(name, stayed, stayed ? `stayed on github (${page.url()})` : `redirected to ${page.url()}`);
  } catch (e) {
...
await expectRedirect('Commit redirect', 'https://github.com/facebook/react/commit/0b08c0e7');
await expectRedirect('Compare (3-dot) redirect', 'https://github.com/facebook/react/compare/v18.2.0...v18.3.0');
await expectRedirect('.diff suffix → base redirect', 'https://github.com/facebook/react/pull/28000.diff');
...
//     per-tab flag doesn't bleed into the redirect checks above ---
{
  ...
  try {
    await esc.goto('https://diffshub.com/facebook/react/pull/28000', { waitUntil: 'domcontentloaded' });
    await esc.waitForFunction(
      () => [...document.querySelectorAll('a[href*="github.com"]')].some((a) => a.href.includes('dh-skip')),
      { timeout: 10000 },
    );
  ...
  await esc.goto('https://github.com/facebook/react/pull/28000?dh-skip=1', { waitUntil: 'domcontentloaded' });
  await esc.waitForTimeout(2500);
  const onGithubClean = esc.url().startsWith('https://github.com/') && !esc.url().includes('dh-skip');
  ...
  record('Escape is sticky across in-page nav', esc.url().startsWith('https://github.com/'), esc.url());
  ...
  await dh.goto('https://diffshub.com/facebook/react/pull/28000', { waitUntil: 'domcontentloaded' });
  ...
  const [ghTab] = await Promise.all([
    ...
    dh.locator('a[href*="github.com"]').first().click().catch(() => {}),
  ]);
  if (ghTab) await ghTab.waitForTimeout(3500);
  const url = ghTab ? ghTab.url() : '';
  const linkOk = url.startsWith('https://github.com/') && url.includes('/pull/28000') && !url.includes('dh-skip');
  record('"View on GitHub" link reaches GitHub (not bounced)', linkOk, url || 'no new tab opened');
...
//     GitHub diff history entry, so Back skips straight past it) ---
{
  ...
  await bk.goto('https://github.com/facebook/react/pulls', { waitUntil: 'domcontentloaded' });
  await bk.waitForTimeout(2000);
  await bk.locator('a[href*="/facebook/react/pull/"]').first().click().catch(() => {});
  for (let i = 0; i < 15; i++) { if (/diffshub\.com/.test(bk.url())) break; await bk.waitForTimeout(700); }
